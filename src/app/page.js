...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen bg-[#050505] text-slate-200 selection:bg-cyan-500/30 font-sans overflow-hidden">
      ...
      <header className="fixed top-0 w-full z-50 bg-[#050505]/60 backdrop-blur-xl border-b border-white/[0.05] shadow-2xl shadow-black/50 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             {/* Dynamic Auth Button Placeholder */}
            <button className="hidden sm:block px-5 py-2.5 rounded-full text-sm font-bold text-white bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
              Sign In
            </button>
            <button className="px-5 py-2.5 rounded-full text-sm font-bold text-[#050505] bg-gradient-to-r from-cyan-400 to-blue-500 hover:shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all duration-300">
              Create Profile
            </button>
          </div>
        </div>
      ...
      <main className="relative pt-32 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="sticky top-24 z-40 bg-[#0a0a0a]/80 backdrop-blur-2xl border border-white/10 rounded-3xl p-2 mb-10 shadow-2xl flex flex-col md:flex-row gap-2 transition-all">
        ...
        {loading ? (
          ...
          /* Premium Cards Grid */
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out">
            {buddies.map(buddy => (
              <BuddyCard key={buddy.id} buddy={buddy} />
            ))}
          </div>
        ) : (
          /* $100k Empty State Design */
          <div className="flex flex-col items-center justify-center py-32 px-4 text-center animate-in fade-in duration-1000">
            ...
            <p className="text-slate-500 max-w-md mx-auto mb-8 font-medium">
              {search || filters.game !== 'All' 
                ? "No operators found with these exact parameters. Try adjusting your search filters to widen the net." 
                : "You are the first one here. Secure your gamertag and create the inaugural profile on the network."}
            </p>
            <button className="px-8 py-3 rounded-full text-sm font-bold text-[#050505] bg-white hover:bg-cyan-400 hover:shadow-[0_0_30px_rgba(34,211,238,0.3)] transition-all duration-300">
